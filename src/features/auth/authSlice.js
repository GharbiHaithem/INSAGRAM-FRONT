        ...
        builder.addCase(createUser.pending, (state) => {
            ...
            .addCase(getAuser.fulfilled, (state, action) => {
                ...
                function removeDuplicateObjects(arr) {
                    ...
                    arr.forEach(item => {
                      if (item !== null && !uniqueObjectsById[item._id]) {
                  ...
                  const combinedPayload = state.chatParPerson.concat(action.payload);

                  // Supprimer les doublons d'objets en utilisant la propriété _id
                  const cleanedPayload = removeDuplicateObjects(combinedPayload);
                  const cleanedArray = cleanedPayload.filter(item => Object.keys(item).length !== 1 || !item.hasOwnProperty('message'));

                  console.log(cleanedArray);
                  state.chatParPerson = cleanedArray;
